...
export default function WhatsAppButton({ whatsapp = "201000000000" }: Props) {
  ...
  const buttonText = lang === "ar" ? "مرحباً، أريد الاستفسار عن منتج" : "Hello, I would like to ask about a product";

  return (
    <a
      href={`https://wa.me/${whatsapp}?text=${encodeURIComponent(buttonText)}`}
      target="_blank"
      rel="noopener noreferrer"
      className="fixed bottom-6 left-6 z-50 bg-[#25D366] text-white p-4 rounded-full shadow-lg hover:shadow-xl hover:scale-110 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#25D366]/30 group"
